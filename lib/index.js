        ...
        "multiline-comment-indent": {
            ...
            create(context) {
                const isValidComment = (node, indent) => {
                    ...
                    const lines = value.slice(2).split("\n");
                    return lines.every((x, i) => {
                ...
                const formatComment = (comment, indent) => {
                    const lines = comment.slice(2).split("\n");
                    const i = " ".repeat(indent);
                    return `/**\n${lines.map((x) => `${i}${x.trimLeft()}`).join("\n")}*/`;
                };
                const checkLeadingComments = (node) => {
                    ...
                    for (const comment of node.leadingComments) {
                        ...
                        if (isValidComment(comment, indent)) {
                            continue;
                        }
                        context.report({
                            node: comment,
                            message: "stars must have identical indent",
                            fix(fixer) {
